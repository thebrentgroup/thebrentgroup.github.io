...
bH.innerHTML="   <link/><table></table><a href='/brentlab-dumps/a' style='top:1px;float:left;opacity:.55;'>a</a><input type='checkbox'/><nav></nav>";
...
(function(h){h.fn.oembed=function(b,c,a){c=h.extend(true,h.fn.oembed.defaults,c);
return this.each(function(){var l=h(this),k=(b!=null)?b:l.attr("href"),d;
if(!a){a=function(i,j){h.fn.oembed.insertCode(i,c.embedMethod,j)
}
}if(k!=null){d=g(k);
if(d!=null){d.maxWidth=c.maxWidth;
d.maxHeight=c.maxHeight;
d.params=c[d.name]||{};
d.embedCode(l,k,a);
return
}}a(l,null)
...
h.fn.oembed.defaults={maxWidth:null,maxHeight:null,embedMethod:"replace"};
h.fn.oembed.insertCode=function(c,b,d){switch(b){case"auto":if(c.attr("href")!=null){insertCode(c,"append",d)
}else{insertCode(c,"replace",d)
}break;
case"replace":c.replaceWith(d.code);
break;
case"fill":c.html(d.code);
break;
case"append":var a=c.next();
if(a==null||!a.hasClass("oembed-container")){a=c.after('<div class="oembed-container"></div>').next(".oembed-container");
if(d!=null&&d.provider_name!=null){a.toggleClass("oembed-container-"+d.provider_name)
}}a.html(d.code);
break
}};
h.fn.oembed.getPhotoCode=function(d,b){var a=b.title?b.title:"";
a+=b.author_name?" - "+b.author_name:"";
a+=b.provider_name?" - "+b.provider_name:"";
var c='<div><a href="'+d+'" target="_blank"><img src="'+b.url+'" alt="'+a+'"/></a></div>';
if(b.html){c+="<div>"+b.html+"</div>"
}return c
};
h.fn.oembed.getVideoCode=function(c,a){var b=a.html;
return b
};
h.fn.oembed.getRichCode=function(c,a){var b=a.html;
return b
};
h.fn.oembed.getGenericCode=function(d,b){var a=(b.title!=null)?b.title:d,c='<a href="'+d+'">'+a+"</a>";
if(b.html){c+="<div>"+b.html+"</div>"
}return c
};
h.fn.oembed.isAvailable=function(b){var a=g(b);
return(a!=null)
};
function g(b){for(var a=0;
a<e.length;
a++){if(e[a].matches(b)){return e[a]
}}return null
}var e=[new f("fivemin","5min.com"),new f("amazon","amazon.com"),new f("flickr","flickr","http://flickr.com/services/oembed","jsoncallback"),new f("googlevideo","video.google."),new f("hulu","hulu.com"),new f("imdb","imdb.com"),new f("metacafe","metacafe.com"),new f("qik","qik.com"),new f("revision3","slideshare"),new f("slideshare","5min.com"),new f("twitpic","twitpic.com"),new f("viddler","viddler.com"),new f("vimeo","vimeo.com","http://vimeo.com/api/oembed.json"),new f("wikipedia","wikipedia.org"),new f("wordpress","wordpress.com"),new f("youtube","youtube.com")];
function f(d,c,a,b){this.name=d;
this.urlPattern=c;
this.oEmbedUrl=(a!=null)?a:"http://oohembed.com/oohembed/";
this.callbackparameter=(b!=null)?b:"callback";
this.maxWidth=500;
this.maxHeight=400;
this.matches=function(j){return j.indexOf(this.urlPattern)>=0
};
this.getRequestUrl=function(i){var o=this.oEmbedUrl;
if(o.indexOf("?")<=0){o=o+"?"
}var p="";
for(var n in this.params){if(n==this.callbackparameter){continue
}if(this.params[n]!=null){p+="&"+escape(n)+"="+this.params[n]
}}o+="format=json";
if(this.maxWidth!=null){o+="&maxwidth="+this.maxWidth
}if(this.maxHeight!=null){o+="&maxheight="+this.maxHeight
}o+="&url="+escape(i)+p+"&"+this.callbackparameter+"=?";
return o
};
this.embedCode=function(p,n,m){var o=this.getRequestUrl(n);
h.getJSON(o,function(j){var i=h.extend(j);
var k,l=j.type;
switch(l){case"photo":i.code=h.fn.oembed.getPhotoCode(n,j);
break;
case"video":i.code=h.fn.oembed.getVideoCode(n,j);
break;
case"rich":i.code=h.fn.oembed.getRichCode(n,j);
break;
default:i.code=h.fn.oembed.getGenericCode(n,j);
break
}m(p,i)
})
}
...
}(window.jQuery),!function(b){var a='[data-dismiss="alert"]',c=function(d){b(d).on("click",a,this.close)
};
c.prototype.close=function(e){function f(){d.trigger("closed").remove()
}var h=b(this),g=h.attr("data-target"),d;
...
},b.fn.alert=function(d){return this.each(function(){var f=b(this),e=f.data("alert");
e||f.data("alert",e=new c(this)),typeof d=="string"&&e[d].call(f)
})
},b.fn.alert.Constructor=c,b(function(){b("body").on("click.alert.data-api",a,c.prototype.close)
})
}(window.jQuery),!function(b){var a=function(c,d){this.$element=b(c),this.options=b.extend({},b.fn.button.defaults,d)
};
a.prototype.setState=function(g){var d="disabled",h=this.$element,f=h.data(),c=h.is("input")?"val":"html";
...
},b.fn.button=function(c){return this.each(function(){var f=b(this),d=f.data("button"),e=typeof c=="object"&&c;
...
}(window.jQuery),!function(b){var a=function(c,d){this.$element=b(c),this.options=d,this.options.slide&&this.slide(this.options.slide),this.options.pause=="hover"&&this.$element.on("mouseenter",b.proxy(this.pause,this)).on("mouseleave",b.proxy(this.cycle,this))
};
...
},to:function(d){var g=this.$element.find(".item.active"),f=g.parent().children(),c=f.index(g),e=this;
if(d>f.length-1||d<0){return
}return this.sliding?this.$element.one("slid",function(){e.to(d)
}):c==d?this.pause().cycle():this.slide(d>c?"next":"prev",b(f[d]))
},pause:function(c){return c||(this.paused=!0),this.$element.find(".next, .prev").length&&b.support.transition.end&&(this.$element.trigger(b.support.transition.end),this.cycle()),clearInterval(this.interval),this.interval=null,this
},next:function(){if(this.sliding){return
}return this.slide("next")
},prev:function(){if(this.sliding){return
}return this.slide("prev")
},slide:function(l,e){var c=this.$element.find(".item.active"),g=e||c[l](),m=this.interval,d=l=="next"?"left":"right",k=l=="next"?"first":"last",j=this,h=b.Event("slide",{relatedTarget:g[0]});
this.sliding=!0,m&&this.pause(),g=g.length?g:this.$element.find(".item")[k]();
if(g.hasClass("active")){return
}if(b.support.transition&&this.$element.hasClass("slide")){this.$element.trigger(h);
if(h.isDefaultPrevented()){return
}g.addClass(l),g[0].offsetWidth,c.addClass(d),g.addClass(d),this.$element.one(b.support.transition.end,function(){g.removeClass([l,d].join(" ")).addClass("active"),c.removeClass(["active",d].join(" ")),j.sliding=!1,setTimeout(function(){j.$element.trigger("slid")
},0)
})
...
}},b.fn.carousel=function(c){return this.each(function(){var f=b(this),d=f.data("carousel"),e=b.extend({},b.fn.carousel.defaults,typeof c=="object"&&c),g=typeof c=="string"?c:e.slide;
d||f.data("carousel",d=new a(this,e)),typeof c=="number"?d.to(c):g?d[g]():e.interval&&d.cycle()
})
},b.fn.carousel.defaults={interval:5000,pause:"hover"},b.fn.carousel.Constructor=a,b(function(){b("body").on("click.carousel.data-api","[data-slide]",function(d){var g=b(this),f,c=b(g.attr("data-target")||(f=g.attr("href"))&&f.replace(/.*(?=#[^\s]+$)/,"")),e=!c.data("modal")&&b.extend({},c.data(),g.data());
c.carousel(e),d.preventDefault()
})
})
}(window.jQuery),!function(b){var a=function(c,d){this.$element=b(c),this.options=b.extend({},b.fn.collapse.defaults,d),this.options.parent&&(this.$parent=b(this.options.parent)),this.options.toggle&&this.toggle()
};
a.prototype={constructor:a,dimension:function(){var c=this.$element.hasClass("width");
return c?"width":"height"
},show:function(){var d,f,e,c;
if(this.transitioning){return
}d=this.dimension(),f=b.camelCase(["scroll",d].join("-")),e=this.$parent&&this.$parent.find("> .accordion-group > .in");
if(e&&e.length){c=e.data("collapse");
...
},transition:function(d,g,f){var c=this,e=function(){g.type=="show"&&c.reset(),c.transitioning=0,c.$element.trigger(f)
};
...
}},b.fn.collapse=function(c){return this.each(function(){var f=b(this),d=f.data("collapse"),e=typeof c=="object"&&c;
d||f.data("collapse",d=new a(this,e)),typeof c=="string"&&d[c]()
})
},b.fn.collapse.defaults={toggle:!0},b.fn.collapse.Constructor=a,b(function(){b("body").on("click.collapse.data-api","[data-toggle=collapse]",function(d){var g=b(this),f,c=g.attr("data-target")||d.preventDefault()||(f=g.attr("href"))&&f.replace(/.*(?=#[^\s]+$)/,""),e=b(c).data("collapse")?"toggle":g.data();
g[b(c).hasClass("in")?"addClass":"removeClass"]("collapsed"),b(c).collapse(e)
})
})
}(window.jQuery),!function(d){function c(){b(d(a)).removeClass("open")
}function b(e){var h=e.attr("data-target"),g;
return h||(h=e.attr("href"),h=h&&/#/.test(h)&&h.replace(/.*(?=#[^\s]*$)/,"")),g=d(h),g.length||(g=e.parent()),g
}var a="[data-toggle=dropdown]",f=function(e){var g=d(e).on("click.dropdown.data-api",this.toggle);
d("html").on("click.dropdown.data-api",function(){g.parent().removeClass("open")
})
};
f.prototype={constructor:f,toggle:function(e){var i=d(this),g,h;
if(i.is(".disabled, :disabled")){return
}return g=b(i),h=g.hasClass("open"),c(),h||(g.toggleClass("open"),i.focus()),!1
},keydown:function(h){var l,j,i,k,g,e;
if(!/(38|40|27)/.test(h.keyCode)){return
}l=d(this),h.preventDefault(),h.stopPropagation();
if(l.is(".disabled, :disabled")){return
}k=b(l),g=k.hasClass("open");
if(!g||g&&h.keyCode==27){return l.click()
...
}},d.fn.dropdown=function(e){return this.each(function(){var h=d(this),g=h.data("dropdown");
...
}b("body").addClass("modal-open"),this.isShown=!0,this.escape(),this.backdrop(function(){var e=b.support.transition&&c.$element.hasClass("fade");
...
b(document).on("focusin.modal",function(d){c.$element[0]!==d.target&&!c.$element.has(d.target).length&&c.$element.focus()
})
},escape:function(){var c=this;
this.isShown&&this.options.keyboard?this.$element.on("keyup.dismiss.modal",function(d){d.which==27&&c.hide()
}):this.isShown||this.$element.off("keyup.dismiss.modal")
},hideWithTransition:function(){var c=this,d=setTimeout(function(){c.$element.off(b.support.transition.end),c.hideModal()
},500);
this.$element.one(b.support.transition.end,function(){clearTimeout(d),c.hideModal()
})
},hideModal:function(c){this.$element.hide().trigger("hidden"),this.backdrop()
},removeBackdrop:function(){this.$backdrop.remove(),this.$backdrop=null
},backdrop:function(d){var f=this,e=this.$element.hasClass("fade")?"fade":"";
if(this.isShown&&this.options.backdrop){var c=b.support.transition&&e;
this.$backdrop=b('<div class="modal-backdrop '+e+'" />').appendTo(document.body),this.options.backdrop!="static"&&this.$backdrop.click(b.proxy(this.hide,this)),c&&this.$backdrop[0].offsetWidth,this.$backdrop.addClass("in"),c?this.$backdrop.one(b.support.transition.end,d):d()
...
},b.fn.modal.defaults={backdrop:!0,keyboard:!0,show:!0},b.fn.modal.Constructor=a,b(function(){b("body").on("click.modal.data-api",'[data-toggle="modal"]',function(d){var g=b(this),f=g.attr("href"),c=b(g.attr("data-target")||f&&f.replace(/.*(?=#[^\s]+$)/,"")),e=c.data("modal")?"toggle":b.extend({remote:!/#/.test(f)&&f},c.data(),g.data());
d.preventDefault(),c.modal(e).one("hide",function(){g.focus()
})
})
})
}(window.jQuery),!function(b){var a=function(d,c){this.init("tooltip",d,c)
};
a.prototype={constructor:a,init:function(d,g,f){var c,e;
...
},enter:function(c){var d=b(c.currentTarget)[this.type](this._options).data(this.type);
...
},leave:function(c){var d=b(c.currentTarget)[this.type](this._options).data(this.type);
this.timeout&&clearTimeout(this.timeout);
if(!d.options.delay||!d.options.delay.hide){return d.hide()
}d.hoverState="out",this.timeout=setTimeout(function(){d.hoverState=="out"&&d.hide()
},d.options.delay.hide)
},show:function(){var h,d,k,g,c,f,j;
if(this.hasContent()&&this.enabled){h=this.tip(),this.setContent(),this.options.animation&&h.addClass("fade"),f=typeof this.options.placement=="function"?this.options.placement.call(this,h[0],this.$element[0]):this.options.placement,d=/in/.test(f),h.remove().css({top:0,left:0,display:"block"}).appendTo(d?this.$element:document.body),k=this.getPosition(d),g=h[0].offsetWidth,c=h[0].offsetHeight;
switch(d?f.split(" ")[1]:f){case"bottom":j={top:k.top+k.height,left:k.left+k.width/2-g/2};
break;
case"top":j={top:k.top-c,left:k.left+k.width/2-g/2};
break;
case"left":j={top:k.top+k.height/2-c/2,left:k.left-g};
break;
case"right":j={top:k.top+k.height/2-c/2,left:k.left+k.width}
}h.css(j).addClass(f).addClass("in")
...
},hide:function(){function d(){var f=setTimeout(function(){e.off(b.support.transition.end).remove()
},500);
e.one(b.support.transition.end,function(){clearTimeout(f),e.remove()
})
}var c=this,e=this.tip();
return e.removeClass("in"),b.support.transition&&this.$tip.hasClass("fade")?d():e.remove(),this
...
},getTitle:function(){var d,c=this.$element,f=this.options;
...
}},b.fn.tooltip=function(c){return this.each(function(){var f=b(this),d=f.data("tooltip"),e=typeof c=="object"&&c;
d||f.data("tooltip",d=new a(this,e)),typeof c=="string"&&d[c]()
})
},b.fn.tooltip.Constructor=a,b.fn.tooltip.defaults={animation:!0,placement:"top",selector:!1,template:'<div class="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',trigger:"hover",title:"",delay:0,html:!0}
}(window.jQuery),!function(b){var a=function(d,c){this.init("popover",d,c)
};
a.prototype=b.extend({},b.fn.tooltip.Constructor.prototype,{constructor:a,setContent:function(){var d=this.tip(),c=this.getTitle(),f=this.getContent();
...
},getContent:function(){var d,c=this.$element,f=this.options;
...
}}),b.fn.popover=function(c){return this.each(function(){var f=b(this),d=f.data("popover"),e=typeof c=="object"&&c;
...
}(window.jQuery),!function(b){function a(d,g){var f=b.proxy(this.process,this),c=b(d).is("body")?b(window):b(d),e;
...
this.offsets=b([]),this.targets=b([]),d=this.$body.find(this.selector).map(function(){var e=b(this),g=e.data("target")||e.attr("href"),f=/^#\w/.test(g)&&b(g);
...
if(h>=k){return f!=(j=c.last()[0])&&this.activate(j)
}for(j=g.length;
j--;
){f!=c[j]&&h>=g[j]&&(!g[j+1]||h<=g[j+1])&&this.activate(c[j])
}},activate:function(c){var e,d;
...
}},b.fn.scrollspy=function(c){return this.each(function(){var f=b(this),d=f.data("scrollspy"),e=typeof c=="object"&&c;
...
}(window.jQuery),!function(b){var a=function(c){this.element=b(c)
};
a.prototype={constructor:a,show:function(){var d=this.element,h=d.closest("ul:not(.dropdown-menu)"),f=d.attr("data-target"),c,e,g;
f||(f=d.attr("href"),f=f&&f.replace(/.*(?=#[^\s]*$)/,""));
if(d.parent("li").hasClass("active")){return
}c=h.find(".active a").last()[0],g=b.Event("show",{relatedTarget:c}),d.trigger(g);
if(g.isDefaultPrevented()){return
}e=b(f),this.activate(d.parent("li"),h),this.activate(e,e.parent(),function(){d.trigger({type:"shown",relatedTarget:c})
})
...
}var c=h.find("> .active"),e=f&&b.support.transition&&c.hasClass("fade");
...
},b.fn.tab.Constructor=a,b(function(){b("body").on("click.tab.data-api",'[data-toggle="tab"], [data-toggle="pill"]',function(c){c.preventDefault(),b(this).tab("show")
})
})
...
return c=b.grep(c,function(f){return d.matcher(f)
...
},sorter:function(g){var d=[],h=[],f=[],c;
while(c=g.shift()){c.toLowerCase().indexOf(this.query.toLowerCase())?~c.indexOf(this.query)?h.push(c):f.push(c):d.push(c)
}return d.concat(h,f)
},highlighter:function(d){var c=this.query.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,"\\$&");
return d.replace(new RegExp("("+c+")","ig"),function(g,f){return"<strong>"+f+"</strong>"
})
},render:function(c){var d=this;
return c=b(c).map(function(e,f){return e=b(d.options.item).attr("data-value",f),e.find("a").html(d.highlighter(f)),e[0]
}),c.first().addClass("active"),this.$menu.html(c),this
},next:function(c){var e=this.$menu.find(".active").removeClass("active"),d=e.next();
d.length||(d=b(this.$menu.find("li")[0])),d.addClass("active")
},prev:function(d){var c=this.$menu.find(".active").removeClass("active"),f=c.prev();
...
break;
case 38:c.preventDefault(),this.prev();
break;
case 40:c.preventDefault(),this.next()
}c.stopPropagation()
},keydown:function(c){this.suppressKeyPressRepeat=!~b.inArray(c.keyCode,[40,38,9,13,27]),this.move(c)
...
},click:function(c){c.stopPropagation(),c.preventDefault(),this.select()
},mouseenter:function(c){this.$menu.find(".active").removeClass("active"),b(c.currentTarget).addClass("active")
}},b.fn.typeahead=function(c){return this.each(function(){var f=b(this),d=f.data("typeahead"),e=typeof c=="object"&&c;
...
}(window.jQuery),!function(b){var a=function(c,d){this.options=b.extend({},b.fn.affix.defaults,d),this.$window=b(window).on("scroll.affix.data-api",b.proxy(this.checkPosition,this)),this.$element=b(c),this.checkPosition()
};
a.prototype.checkPosition=function(){if(!this.$element.is(":visible")){return
}var f=b(document).height(),k=this.$window.scrollTop(),h=this.$element.offset(),e=this.options.offset,g=e.bottom,j=e.top,d="affix affix-top affix-bottom",c;
...
if(this.affixed===c){return
}this.affixed=c,this.unpin=c=="bottom"?h.top-k:null,this.$element.removeClass(d).addClass("affix"+(c?"-"+c:""))
},b.fn.affix=function(c){return this.each(function(){var f=b(this),d=f.data("affix"),e=typeof c=="object"&&c;
d||f.data("affix",d=new a(this,e)),typeof c=="string"&&d[c]()
})
},b.fn.affix.Constructor=a,b.fn.affix.defaults={offset:0},b(window).on("load",function(){b('[data-spy="affix"]').each(function(){var c=b(this),d=c.data();
...
$(document).ready(function(){$(function(){$("#myCarousel").carousel({interval:5000})
})
});
$(document).ready(function(){jQuery(".carousel-icons").click(function(){var imgname=jQuery(this).attr("value");
...
jQuery(".carousel-control.right").click(function(){var plcid=jQuery(".active").attr("id");
...
if(nextImageNumber>numberOfImages){nextImageNumber=1;
plcid="img"+numberOfImages
}if(nextImageNumber<=numberOfImages){jQuery("."+plcid).find(".placeholder").attr("src","/etc/designs/default/0.gif");
jQuery("."+plcid).removeClass("current");
...
jQuery(".carousel-control.left").click(function(){var plcid=jQuery(".active").attr("id");
var numberOfImages=jQuery(".carousel-icons").children().size();
var currentImageNumber=plcid.substring(3);
var nextImageNumber=eval(currentImageNumber)-1;
if(nextImageNumber==0){nextImageNumber=numberOfImages;
...
jQuery("#myCarousel").bind("slid",function(){var plcid=jQuery(".active").attr("id");
var numberOfImages=jQuery(".carousel-icons").children().size();
var currentImageNumber=plcid.substring(3);
var previousImageNumber=eval(currentImageNumber)-1;
if(previousImageNumber==0){previousImageNumber=numberOfImages;
plcid="img"+previousImageNumber
}else{plcid="img"+previousImageNumber
}jQuery("."+plcid).find(".placeholder").attr("src","/etc/designs/default/0.gif");
jQuery("."+plcid).removeClass("current");
...
jQuery(document).ready(function(){jQuery(".dropdown").hover(function(){var a=this;
jQuery(".dropdown").removeClass("open");
var b=jQuery(a).attr("editMode");
if(b=="true"||jQuery(this).find("li").size()>0){jQuery(this).addClass("open")
}},function(){var a=this;
var b=jQuery(a).attr("editMode");
if(b=="false"){jQuery(this).removeClass("open")
}})
});
jQuery(document).ready(function(){jQuery(document).on("mouseleave","#myCarousel2",function(){jQuery(this).carousel("pause")
});
jQuery(".slide-thumb").click(function(){if(jQuery("#thumbs-pointer").is(":animated")){return false
}else{var a=jQuery(".slide-thumb.current").position();
var c=jQuery(this).position();
var e=jQuery("#thumbs-pointer").css("left");
var g="#contentdiv"+jQuery(this).closest("li").attr("pos");
var f="#contentdiv"+jQuery(".slide-thumb.current").closest("li").attr("pos");
var h="";
if(a.left<c.left){h="+="+(c.left-a.left)
}else{h="-="+(a.left-c.left)
}jQuery("#thumbs-pointer").animate({left:h},500,function(){});
jQuery(f).addClass("disabled");
jQuery(".current").removeClass("current");
jQuery(g).removeClass("disabled");
jQuery(this).addClass("current");
var d=jQuery(this).attr("pos");
var i=d-1;
jQuery("#myCarousel2").carousel(i);
b();
return false
}function b(){var j=jQuery(window).height();
var l=100;
var k=jQuery("#footer").position().top+l;
jQuery("#footer").css("margin-top","0px");
if(k<j){jQuery("#footer").css("margin-top",15+(j-k)+"px")
}}})
});
(function(b,a){b.fn.bootpag=function(d){var g=this,e=b.extend({total:0,page:1,maxVisible:null,leaps:true,href:"javascript:void(0);",hrefVariable:"{{number}}",next:"&raquo;",prev:"&laquo;"},g.data("settings")||{},d||{});
if(e.total<=0){return this
}if(!e.maxVisible){e.maxVisible=e.total
}g.data("settings",e);
function f(j,l){var i,k=Math.floor(l/e.maxVisible)*e.maxVisible,h=j.find("li");
e.page=l=l<0?0:l>e.total?e.total:l;
h.removeClass("disabled");
i=l-1<1?1:e.leaps&&l-1>e.maxVisible?Math.floor((l-1)/e.maxVisible)*e.maxVisible:l-1;
h.first().toggleClass("disabled",l===1).attr("data-lp",i).find("a").attr("href",c(i));
i=l+1>e.total?e.total:e.leaps&&l+1<e.total-e.maxVisible?k+e.maxVisible+1:l+1;
h.last().toggleClass("disabled",l===e.total).attr("data-lp",i).find("a").attr("href",c(i));
var m=h.filter("[data-lp="+l+"]");
if(!m.not(".next,.prev").length){var n=l<=k?-e.maxVisible:0;
h.not(".next,.prev").each(function(o){i=o+1+k+n;
b(this).attr("data-lp",i).toggle(i<=e.total).find("a").html(i).attr("href",c(i))
});
m=h.filter("[data-lp="+l+"]")
}m.addClass("disabled");
g.trigger("page",l);
g.data("settings",e)
}function c(h){return e.href.replace(e.hrefVariable,h)
}return this.each(function(){var j,i,k=b(this),l=['<ul class="bootpag">'];
if(e.prev){l.push('<li data-lp="1" class="prev"><a href="'+c(1)+'">'+e.prev+"</a></li>")
}for(var m=1;
m<=Math.min(e.total,e.maxVisible);
m++){l.push('<li data-lp="'+m+'"><a href="'+c(m)+'">'+m+"</a></li>")
}if(e.next){i=e.leaps&&e.total>e.maxVisible?Math.min(e.maxVisible+1,e.total):2;
l.push('<li data-lp="'+i+'" class="next"><a href="'+c(i)+'">'+e.next+"</a></li>")
}l.push("</ul>");
k.find("ul.bootpag").remove();
k.append(l.join("")).addClass("pagination");
j=k.find("ul.bootpag");
k.find("li").click(function h(){var n=b(this);
